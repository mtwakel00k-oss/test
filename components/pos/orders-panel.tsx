...
interface RawOrder {
  ...
  order_type: string; payment_status: string; customer_name: string; customer_phone: string | null
  delivery_lat?: number | null; delivery_lng?: number | null; delivery_address?: string | null
  driver_id?: string | null; total: number | string; created_at: string; items?: RawOrderItem[]
}

interface RawOrderItem {
  id: string; order_id: string; product_id: number; product_name: string
  size: string | null; sauce: number | null; unit_price: number; quantity: number
}
...
function toPosOrder(raw: RawOrder, items: RawOrderItem[], dailyIndex?: number, allDrivers?: { id: string; name: string; phone: string }[]): PosOrder {
  const orderType = (["dine_in","takeaway","delivery"].includes(raw.order_type) ? raw.order_type : "dine_in") as PosOrder["orderType"]
  const driver = allDrivers?.find(d => d.id === raw.driver_id)
  return {
    id: raw.id, orderNumber: raw.order_number ?? (dailyIndex != null ? dailyIndex + 1 : null), tableNumber: raw.table_number ?? null, orderType,
    status: (statusMap[raw.status] as PosOrderStatus) || "pending", paymentStatus: (raw.payment_status as PosOrder["paymentStatus"]) || "unpaid",
    serverName: raw.customer_name || "زائر", customerPhone: String(raw.customer_phone ?? ""),
    deliveryLat: raw.delivery_lat ?? null, deliveryLng: raw.delivery_lng ?? null, deliveryAddress: raw.delivery_address ?? null,
    driverId: raw.driver_id ?? null, driverName: driver?.name ?? null, driverPhone: driver?.phone ?? null,
    items: items.map((i) => ({
      id: i.id, name: i.product_name + (i.size && i.size !== "UNIQUE" ? ` (${i.size})` : ""),
      quantity: i.quantity, price: Number(i.unit_price), productId: i.product_id, size: i.size, sauce: i.sauce,
    })),
    total: Number(raw.total), createdAt: new Date(raw.created_at),
  }
}
...
type UnifiedDriver = { id: string; name: string; phone: string; isBusy: boolean; source: "tenant" | "delivery_men" }
...
export function OrdersPanel({ onNewOrder }: { onNewOrder: () => void }) {
  ...
  const fetchOrders = useCallback(async (): Promise<PosOrder[]> => {
    ...
    const dailyOrders = [...rawOrders].filter((o) => o.created_at?.startsWith(today))
      .sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime())
    const dailyIndex = new Map(dailyOrders.map((o, i) => [o.id, i]))
    const itemsByOrder: Record<string, RawOrderItem[]> = {}
    for (const o of rawOrders) { if (o.items) itemsByOrder[o.id] = o.items }
    return rawOrders.map((o) => toPosOrder(o, itemsByOrder[o.id] || [], dailyIndex.get(o.id), allDrivers))
      .filter((o: PosOrder) => !cancelledIds.has(o.id))
  }, [allDrivers])

  useEffect(() => { fetchOrders().then(data => startTransition(() => setOrders(data))) }, [fetchOrders])

  const debouncedRefresh = useMemo(() => debounce(() => { fetchOrders().then(data => startTransition(() => setOrders(data))) }, 350), [fetchOrders])
  useEffect(() => () => debouncedRefresh.cancel(), [debouncedRefresh])

  const subscriptions = useMemo(() => [
    { table: "orders" as const, event: "*" as const, handler: () => { debouncedRefresh() } },
    { table: "order_items" as const, event: "*" as const, handler: () => { debouncedRefresh() } },
  ], [debouncedRefresh])

  useRealtime({
    channelName: "pos-orders", subscriptions, pollInterval: 10000,
  ...
  const handleStatusChange = useCallback(async (orderId: string | number, status: PosOrderStatus) => {
    const prevOrders = orders; const dbStatus = POS_STATUS_TO_DB[status]
    setOrders(prev => prev.map(o => o.id === orderId ? { ...o, status } : o))
    setSelectedOrder(prev => prev?.id === orderId ? { ...prev, status } : prev)
    const res = await fetchApi(`/api/orders/${orderId}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ status: dbStatus }) })
    if (!res.ok) {
      ...
      toast({ variant: "destructive", title: "فشل تحديث الحالة" }); logger.error("Status update failed")
    }
  }, [orders])
  ...
  const handleComplete = useCallback(async (orderId: string | number, paid: number, change: number, onError: () => void, driverId?: string | null) => {
    ...
    if (isDelivery && !driverId) { toast({ title: "يرجى اختيار سائق", variant: "destructive" }); onError(); return }
    const dbStatus = POS_STATUS_TO_DB[isDelivery ? "out_for_delivery" : "completed"]
    setOrders(p => p.map(o => o.id === orderId ? { ...o, status: isDelivery ? "out_for_delivery" : "completed", ...(isDelivery ? {} : { paymentStatus: "paid" as const }), driverId: driverId ?? o.driverId } : o))
    setSelectedOrder(p => p?.id === orderId ? { ...p, status: isDelivery ? "out_for_delivery" : "completed", ...(isDelivery ? {} : { paymentStatus: "paid" as const }), driverId: driverId ?? p?.driverId } : p)
    const res = await fetchApi(`/api/orders/${orderId}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ status: dbStatus, ...(isDelivery ? {} : { payment_status: "paid" }), ...(isDelivery && driverId ? { driver_id: driverId } : {}) }) })
    if (!res.ok) {
  ...
  const handleCancel = useCallback(async (orderId: string | number) => {
    const snapshot = orders; const cancelledOrder = orders.find(o => o.id === orderId) ?? null
  ...
  const handleSaveItems = useCallback(async (orderId: string | number) => {
    ...
    setSavingItems(true)
    const keptItems = order.items.filter(i => (editQuantities[i.id] ?? i.quantity) > 0).map(i => ({
      product_id: i.productId, product_name: i.name.replace(/\s*\([^)]*\)\s*$/, ''), size: i.size || "UNIQUE",
      sauce: i.sauce, quantity: editQuantities[i.id] ?? i.quantity, unit_price: i.price,
    }))
    const newTotal = keptItems.reduce((s, i) => s + i.unit_price * i.quantity, 0)
    const res = await fetchApi(`/api/orders/${orderId}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ items: keptItems, total: newTotal }) })
    if (res.ok) { setEditingItems(false); setSavingItems(false); setOrders(await fetchOrders()) }
    else { setSavingItems(false); logger.error("Failed to save items") }
  ...
  const assignDriver = useCallback(async (orderId: string | number, driver: UnifiedDriver | null) => {
    ...
    if (driver && !selectedDriver) { toast({ title: "اختر سائقاً من القائمة", variant: "destructive" }); setAssigningDriver(false); return }
    if (driver && selectedDriver?.isBusy) { toast({ title: `السائق ${selectedDriver.name} مشغول حالياً، اختر سائقاً آخر`, variant: "destructive" }); setAssigningDriver(false); return }
    if (driver?.source === "delivery_men") {
      const res = await fetchApi("/api/admin/assign-delivery", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ order_id: String(orderId), delivery_man_id: driver.id }) })
      if (res.ok) {
        ...
        fetchApi("/api/delivery-men").then(r => r.ok ? r.json() : []).then(data => setDeliveryMen(Array.isArray(data) ? data : [])).catch(() => {})
        fetchApi("/api/tenant/drivers").then(r => r.ok ? r.json() : []).then(data => setDrivers(Array.isArray(data) ? data : [])).catch(() => {})
      } else { const err = await res.json().catch(() => ({ error: "فشل تعيين السائق" })); toast({ title: err.error || "فشل تعيين السائق", variant: "destructive" }) }
    } else {
      const res = await fetchApi(`/api/orders/${orderId}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ driver_id: driver?.id ?? null, ...(driver ? { status: POS_STATUS_TO_DB["out_for_delivery"] } : {}) }) })
      if (res.ok) {
  ...
  const filteredOrders = useMemo(() => (activeTab === "active" ? activeOrders : completedOrders).filter(o => !statusFilter || o.status === statusFilter), [activeTab, activeOrders, completedOrders, statusFilter])

  const counts = {
    active: activeOrders.length, completed: completedOrders.length, all: orders.length,
  ...
  return (
    <>
            ...
            <div className="h-full flex flex-col">
              <div className="flex items-center justify-between p-4 border-b border-border">
                ...
                <div className="flex items-center gap-2">
                  <button onClick={handlePrint} className="px-3 py-1.5 rounded-lg text-sm font-medium border border-border text-muted-foreground hover:bg-secondary transition-colors">{t("common.print")}</button>
                  <div className="relative group">
                    <button onClick={() => setShowCheckout(true)} disabled={selectedOrder.status !== "completed"}
                      className="px-3 py-1.5 rounded-lg text-sm font-medium bg-primary text-primary-foreground hover:bg-primary/90 disabled:opacity-40 disabled:cursor-not-allowed">{selectedOrder.status === "completed" ? t("pos.paymentCash") : "غير جاهز"}</button>
                  ...
                  <button onClick={() => { setSelectedOrder(null); setShowCheckout(false) }} className="p-1.5 rounded-lg hover:bg-muted transition-colors">
              ...
              {showCheckout ? (
                <CheckoutPanel order={selectedOrder} onClose={() => setShowCheckout(false)} onComplete={handleComplete}
                  hasDriverAssigned={!!selectedOrder.driverId} drivers={allDrivers} />
                ...
                <div id="receipt" className="flex-1 overflow-y-auto p-4 space-y-3">
                  ...
                  <div className="pt-3 border-t border-border space-y-1">
                    <div className="flex justify-between text-sm text-muted-foreground"><span>{t("pos.total")}</span><span className="text-foreground font-semibold">{selectedOrder.total} {cur}</span></div>
                    <div className="flex justify-between text-sm text-muted-foreground"><span>{t("pos.amountPaid")}</span><span className="text-foreground">{receiptData.paid} {cur}</span></div>
                    <div className="flex justify-between text-sm text-muted-foreground"><span>{t("pos.change")}</span><span className="text-emerald-600 font-semibold">{receiptData.change.toFixed(2)} {cur}</span></div>
                  </div>
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-3">
                  ...
                  <div className="pt-3 border-t border-border flex gap-2">
                    <button onClick={() => { setEditingItems(false); setEditQuantities({}) }} className="flex-1 rounded-lg border border-border py-2.5 text-sm font-medium text-muted-foreground hover:bg-secondary">{t("common.cancel")}</button>
                    <button onClick={() => handleSaveItems(selectedOrder.id)} disabled={savingItems} className="flex-1 rounded-lg bg-primary text-primary-foreground py-2.5 text-sm font-semibold hover:bg-primary/90 disabled:opacity-50">
                      {savingItems ? (<span className="flex items-center justify-center gap-2"><svg className="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" /></svg>{t("common.processing")}</span>) : t("common.save")}
                    </button>
                  </div>
                </div>
              ) : (
                <div className="flex-1 overflow-y-auto p-4 space-y-3">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    {selectedOrder.status !== "completed" && selectedOrder.status !== "cancelled" && (
                      <button onClick={() => { setEditingItems(true); setEditQuantities({}) }} className="text-xs font-medium text-primary hover:underline">{t("pos.editItems")}</button>
                    )}
                  </div>
                  {selectedOrder.items.map(item => (
                    <div key={item.id} className="flex justify-between items-center py-2 border-b border-border/30 last:border-0">
                      <div><p className="text-sm font-medium text-foreground">{item.name}</p><p className="text-xs text-muted-foreground">{t("pos.qty")} {item.quantity}</p></div>
                  ...
                  {selectedOrder.orderType === "delivery" && (selectedOrder.status === "ready" || selectedOrder.status === "preparing") && features?.hasDelivery !== false && (
                    <div className="border-t border-border/50 pt-3 space-y-3">
                      <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide flex items-center gap-1.5">
                        <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16V6a1 1 0 00-1-1H4a1 1 0 00-1 1v10l2-1m2 1l2-1m2 1l2-1m2 1V6a1 1 0 00-1-1h-2a1 1 0 00-1 1v10" /></svg>
                        {t("pos.delivery")}
                      </p>
                        ...
                        return (
                          <div className="flex items-center justify-between rounded-xl border border-border bg-card px-4 py-3">
                            <div className="flex items-center gap-3 min-w-0 flex-1">
                              <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-primary/10 text-primary text-sm font-bold">{assignedDriver ? assignedDriver.name.charAt(0) : "?"}</div>
                              <div className="min-w-0"><p className="text-sm font-semibold text-foreground truncate">{assignedDriver ? assignedDriver.name : "—"}</p><p className="text-xs text-muted-foreground font-mono dir-ltr text-left truncate">{assignedDriver ? assignedDriver.phone : ""}</p></div>
                            </div>
                            <div className="flex items-center gap-1.5 shrink-0">
                              <a href={`tel:${assignedDriver?.phone || ""}`} className="flex items-center justify-center h-9 w-9 rounded-lg bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 hover:bg-emerald-500/20 transition-all">
                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg>
                              </a>
                              <button onClick={async () => { await assignDriver(selectedOrder.id, null) }} disabled={assigningDriver} className="flex items-center justify-center h-9 w-9 rounded-lg text-muted-foreground hover:text-destructive hover:bg-destructive/5 transition-all disabled:opacity-50">
                          ...
                          {allDrivers.filter(d => !d.isBusy).length === 0 ? (
                            ...
                            allDrivers.filter(d => !d.isBusy).map(driver => (
                              <button key={driver.id} onClick={() => assignDriver(selectedOrder.id, driver)} disabled={assigningDriver}
                                ...
                                <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 text-sm font-bold group-hover:bg-emerald-500/20 transition-colors">{driver.name.charAt(0)}</div>
                                <div className="flex-1 min-w-0"><p className="text-sm font-medium text-foreground">{driver.name}</p><p className="text-[11px] text-muted-foreground font-mono dir-ltr text-left">{driver.phone}</p></div>
                                <span className="flex items-center gap-1.5 shrink-0"><span className="w-1.5 h-1.5 rounded-full bg-emerald-500" /><span className="text-[10px] font-semibold text-emerald-600 dark:text-emerald-400">متاح</span></span>
                              </button>
                            ))
                          )}
                        </div>
                  ...
                  <div className="pt-3 border-t border-border">
                    <div className="flex justify-between items-center"><span className="text-base font-semibold text-foreground">{t("pos.total")}</span><span className="text-xl font-bold text-foreground">{selectedOrder.total} {cur}</span></div>
                  </div>
                  {selectedOrder.status !== "completed" && selectedOrder.status !== "cancelled" && (
      ...
      <ConfirmDialog open={cancelTargetId !== null} title={t("pos.cancelOrder")} message={t("pos.cancelConfirm")}
        confirmLabel={t("common.cancel")} cancelLabel={t("pos.keep")}
