...
export default function OrderError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  useEffect(() => { console.error("Order tracking error:", error) }, [error])
  return (
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      <div className="w-full max-w-sm bg-card border border-border rounded-2xl p-8 shadow-lg text-center space-y-4">
        ...
        <h1 className="text-xl font-bold text-foreground">Order Tracking Error</h1>
        <p className="text-sm text-muted-foreground">Something went wrong loading your order</p>
