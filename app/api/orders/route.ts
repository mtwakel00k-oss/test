...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { items, customer_name, table_number, idempotency_key } = body
    ...
    // ── Column-aware insert ─────────────────────────────
    const OPTIONAL_COLS = ["payment_status", "order_type", "order_number", "idempotency_key", "delivery_address", "delivery_lat", "delivery_lng"]
